extends Node
class_name RTSMultiplayerBridge

signal status_changed(message: String)
signal match_started_changed(started: bool)
signal pause_state_changed(paused: bool)
signal spawn_unit_requested(peer_id: int, slot: int, color: Color)
...
func host_game(port: int) -> void:
	...
	if err != OK:
		...
		_emit_status("Host failed (Code %d - %s)" % [err, error_string(err)])
		return
	_last_error_code = OK
	multiplayer.multiplayer_peer = peer
	_match_started = false
	_set_pause_state(false)
	_emit_status("Host active on port %d" % port)
...
func join_game(address: String, port: int) -> void:
	...
	if err != OK:
		_last_error_code = err
		_emit_status("Join failed (Code %d - %s)" % [err, error_string(err)])
		return
	_last_error_code = OK
	multiplayer.multiplayer_peer = peer
	_match_started = false
...
func start_match_as_host() -> void:
	if not multiplayer.is_server():
		_emit_status("Only the host can start.")
		return
	...
	for i in range(peers.size()):
		var peer_id := peers[i]
		var color := DEFAULT_COLORS[i % DEFAULT_COLORS.size()]
		spawn_unit_requested.emit(peer_id, i, color)
		register_packet_sent(1)
		rpc("_rpc_spawn_unit", peer_id, i, color)
	...
	_emit_status("Match started with %d players." % peers.size())
...
func register_packet_sent(count: int = 1) -> void:
	_packets_sent += count


func register_packet_received(count: int = 1) -> void:
...
func _on_peer_connected(id: int) -> void:
	...
	if peer_count > MAX_PLAYERS:
		...
		return
	_emit_status("Peer %d joined (%d/%d)" % [id, peer_count, MAX_PLAYERS])
	if _match_started:
		(multiplayer.multiplayer_peer as ENetMultiplayerPeer).disconnect_peer(id, true)
		_emit_status("Match already running, join rejected.")


func _on_peer_disconnected(id: int) -> void:
	peer_left.emit(id)
	_emit_status("Peer %d disconnected." % id)
...
func _on_connection_failed() -> void:
	...
	_emit_status("Connection failed (Code %d - %s)." % [_last_error_code, error_string(_last_error_code)])
	disconnect_game(false)


func _on_server_disconnected() -> void:
	_last_error_code = ERR_CONNECTION_ERROR
	_emit_status("Host disconnected (Code %d - %s)." % [_last_error_code, error_string(_last_error_code)])
	disconnect_game(false)


@rpc("authority", "reliable")
func _rpc_spawn_unit(peer_id: int, slot: int, color: Color) -> void:
	register_packet_received(1)
	spawn_unit_requested.emit(peer_id, slot, color)
...
func _rpc_restart_notice() -> void:
	register_packet_received(1)
	_emit_status("Match restarted by host.")
...
func _set_pause_state(value: bool) -> void:
	...
	_emit_status("Paused." if _game_paused else "Resumed.")
