...
func get_cell_center_height(cell: Dictionary, corner_heights: Dictionary) -> float:
	# "height" ist der echte Mittelpunkt der Tile-Fläche.
	# Falls eine alte/ungültige Zelle keinen height-Wert hat, wird sauber
	# aus den vier Corner-Werten gemittelt.
	if cell.has("height"):
		return float(cell.get("height", 0.0))

	return (
		float(corner_heights["h_sw"])
		+ float(corner_heights["h_se"])
		+ float(corner_heights["h_ne"])
		+ float(corner_heights["h_nw"])
	) * 0.25
...
func add_cell_top(st: SurfaceTool, local_x: int, local_y: int, cell: Dictionary) -> void:
	...
	var heights := get_cell_corner_heights(cell)
	var center_height := get_cell_center_height(cell, heights)
	...
	var z0 := local_y * tile_size
	var z1 := (local_y + 1) * tile_size
	var cx := x0 + tile_size * 0.5
	var cz := z0 + tile_size * 0.5
	...
	var v_center := Vector3(cx, center_height, cz)
	...
	# Topfläche wird jetzt nicht mehr als ein Quad mit Diagonale gebaut,
	# sondern als 4 Dreiecke über den echten Center-Punkt.
	# Dadurch kann "height" als Mittelpunkt-Höhe benutzt werden,
	# während die vier Corner-Werte die Ränder definieren.
	#
	# Winding bleibt passend zur bisherigen add_quad()-Logik:
	# alte Dreiecke waren v_sw,v_ne,v_se und v_sw,v_nw,v_ne.
	add_triangle(st, v_sw, v_center, v_se, uv_sw, uv_center, uv_se, color)
	add_triangle(st, v_se, v_center, v_ne, uv_se, uv_center, uv_ne, color)
	add_triangle(st, v_ne, v_center, v_nw, uv_ne, uv_center, uv_nw, color)
	add_triangle(st, v_nw, v_center, v_sw, uv_nw, uv_center, uv_sw, color)
